...
from datasets.hea_4000 import Hea4000
...
def get_args_parser():
    parser = argparse.ArgumentParser('Training equivariant networks on HEA', add_help=False)
...
def main(args):
    ...
    ''' Dataset '''
    train_dataset = Hea4000(args.data_path, 'train')
    val_dataset = Hea4000(args.data_path, 'valid')
    test_dataset  = Hea4000(args.data_path, 'test')
    ...
    lengths_dataset = torch.load(os.path.join(args.data_path,"lengths.pt"))

    if args.fold != args.kfold-1:
        train_dataset = train_dataset[torch.sum(lengths_dataset["train"][:args.fold])*2:torch.sum(lengths_dataset["train"][:args.fold+1])*2]
        val_dataset = val_dataset[torch.sum(lengths_dataset["valid"][:args.fold])*2:torch.sum(lengths_dataset["valid"][:args.fold+1])*2]
        test_dataset = test_dataset[torch.sum(lengths_dataset["test"][:args.fold])*2:torch.sum(lengths_dataset["test"][:args.fold+1])*2]
    else:
        train_dataset = train_dataset[-lengths_dataset["train"][args.fold]*2:]
        val_dataset = val_dataset[-lengths_dataset["valid"][args.fold]*2:]
        test_dataset = test_dataset[-lengths_dataset["test"][args.fold]*2:]
    _log.info('')
    _log.info('Training set size:   {}'.format(len(train_dataset)))
    _log.info('Validation set size: {}'.format(len(val_dataset)))
    _log.info('Testing set size:    {}\n'.format(len(test_dataset)))
    ...
    mean = float(y.mean())
    std = float(y.std())
    # _log.info('Training set mean: {}, std: {}'.format(mean, std))

    task_mean, task_std = 0, 1
    if args.standardize:
        task_mean, task_std = mean, std
    ...
    best_metrics = {'val_epoch': 0}
    best_ema_metrics = {'val_epoch': 0}
    if args.energy_weight != 0:
        best_metrics['test_energy_err'] = float('inf')
        best_metrics['val_energy_err'] = float('inf')
        best_ema_metrics['test_energy_err'] = float('inf')
        best_ema_metrics['val_energy_err'] = float('inf')
    if args.force_weight != 0:
        best_metrics['val_force_err'] = float('inf')
        best_metrics['test_force_err'] = float('inf')
        best_ema_metrics['val_force_err'] = float('inf')
        best_ema_metrics['test_force_err'] = float('inf')

    if args.evaluate:
        test_err, test_loss = evaluate(args=args, model=model,  
    ...
    for epoch in range(args.epochs):
        ...
        train_err, train_loss = train_one_epoch(args=args, model=model, 
        ...
        val_err, val_loss = evaluate(args=args, model=model, 
        ...
        if update_val_result:
            torch.save(
                ...
                os.path.join(args.output_dir, 
                    'best_val_epochs@{}.pth.tar'.format(epoch))
            )
        if update_test_result:
            torch.save(
                model, 
                os.path.join(args.output_dir, 
                    'best_test_epochs@{}.pth.tar'.format(epoch))
            )
        if args.energy_weight == 0:
            info_str = 'Epoch: [{epoch}] Target: [{target}] train_MAE: {train_f_mae:.5f}, '.format(
            epoch=epoch, target=args.target,  train_f_mae=train_err['force'].avg)
            info_str += ' val_MAE: {:.5f}, '.format( val_err['force'].avg)
        elif args.force_weight == 0:
            info_str = 'Epoch: [{epoch}] Target: [{target}] train_MAE: {train_e_mae:.5f}, '.format(
            epoch=epoch, target=args.target, train_e_mae=train_err['energy'].avg)
            info_str += 'val_MAE: {:.5f}, '.format(val_err['energy'].avg)
        else:
        ...
        if (epoch + 1) % args.test_interval == 0:
            if args.energy_weight == 0:
                info_str += 'test_MAE: {:.5f}, '.format(test_err['force'].avg)
            elif args.force_weight == 0:
                info_str += 'test_MAE: {:.5f}, '.format(test_err['energy'].avg)
            else:
        ...
        info_str = 'Best -- epoch:{}, '.format(best_metrics['val_epoch'])
        if args.energy_weight == 0:
            info_str += 'val_MAE: {:.5f}, '.format(best_metrics['val_force_err'])
            info_str += 'test_MAE: {:.5f}\n'.format(best_metrics['test_force_err'])
        elif args.force_weight == 0:
            info_str += 'val_MAE: {:.5f}, '.format(best_metrics['val_energy_err'])
            info_str += 'test_MAE: {:.5f}\n'.format(best_metrics['test_energy_err'])
        else:
        ...
        if model_ema is not None:
            ema_val_err, _ = evaluate(args=args, model=model_ema.module,  
                data_loader=val_loader, device=device,
                print_freq=args.print_freq, logger=_log, print_progress=False)
            
            if (epoch + 1) % args.test_interval == 0:
                ema_test_err, _ = evaluate(args=args, model=model_ema.module, 
                    data_loader=test_loader, device=device,
                    print_freq=args.print_freq, logger=_log, print_progress=True, max_iter=-1)
            else:
            ...
            if update_val_result:
                torch.save(
                    model_ema, 
                    os.path.join(args.output_dir, 
                        'best_ema_val_epochs@{}.pth.tar'.format(epoch))
                )
            if update_test_result:
                torch.save(
                    model_ema, 
                    os.path.join(args.output_dir, 
                        'best_ema_test_epochs@{}.pth.tar'.format(epoch))
                )
            if (epoch + 1) % args.test_interval == 0 and (not update_val_result) and (not update_test_result):
                torch.save(
                    model_ema, 
                    os.path.join(args.output_dir, 
                        'ema_epochs@{}.pth.tar'.format(epoch))
                )
...
def update_best_results(args, best_metrics, val_err, test_err, epoch):
    ...
    if args.energy_weight == 0:
        new_loss = val_err['force'].avg
        prev_loss = best_metrics['val_force_err']
        if new_loss < prev_loss:
            best_metrics['val_force_err']  = val_err['force'].avg
            best_metrics['val_epoch'] = epoch
            update_val_result = True
    elif args.force_weight == 0:
        new_loss = val_err['energy'].avg
        prev_loss = best_metrics['val_energy_err']
        if new_loss < prev_loss:
...
def train_one_epoch(args, 
    ...
    if args.force_weight != 0 :
        loss_metrics['force'] = AverageMeter()
        mae_metrics['force'] = AverageMeter()
    ...
    for step, data in enumerate(data_loader):
        ...
        loss = 0
        if args.energy_weight != 0:
            loss_e = F.huber_loss(pred_y.flatten(), data.y, reduction='none')
            loss_e = loss_e.mean()
            loss += args.energy_weight * loss_e
        if args.force_weight != 0:
            loss_f = F.huber_loss(pred_dy.flatten(), data.force, reduction='none')
            loss_f = loss_f.mean()
        ...
        if args.energy_weight != 0:
            ...
            # energy_err = pred_y.detach() * task_std - data.y
        ...
        if args.force_weight != 0:
            ...
            # force_err = pred_dy.detach() * task_std - data.force
        ...
        if step % print_freq == 0 or step == len(data_loader) - 1: 
            ...
            if args.energy_weight == 0:
                info_str +=  'loss: {loss_f:.5f}, MAE: {f_mae:.5f}, '.format(
                    loss_f=loss_metrics['force'].avg, 
                    f_mae=mae_metrics['force'].avg, 
                )
            elif args.force_weight == 0:
                info_str +=  'loss: {loss_e:.5f},  MAE: {e_mae:.5f},  '.format(
...
def evaluate(args, 
    ...
    with torch.no_grad():
        ...
        for step, data in enumerate(data_loader):
            ...
            pred_y, pred_dy = model(data)
            if args.energy_weight != 0:
                loss_e = F.huber_loss(pred_y.flatten(), (data.y-task_mean) / task_std, reduction='none')
                loss_e = loss_e.mean()
                loss_metrics['energy'].update(loss_e.item(), n=pred_y.shape[0])
                energy_err = pred_y.detach() * task_std - data.y
                energy_err = torch.mean(torch.abs(energy_err)).item()
                mae_metrics['energy'].update(energy_err, n=pred_y.shape[0])
            if args.force_weight != 0:
                loss_f = F.huber_loss(pred_dy.flatten() * task_std, data.force, reduction='none')
                loss_f = loss_f.mean()
                loss_metrics['force'].update(loss_f.item(), n=pred_dy.shape[0])
                force_err = pred_dy.detach() * task_std - data.force
...
def mainfold(args):
    ...
    for i in range(args.kfold):
        ...
        _log.info('')
        _log.info('Total Training set size:   {}'.format(len(train_dataset)))
        _log.info('Total Validation set size: {}'.format(len(val_dataset)))
        _log.info('Total Testing set size:    {}\n'.format(len(test_dataset)))
        lengths_dataset = torch.load(os.path.join(args.data_path,"lengths.pt"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser('Training equivariant networks on HEA', parents=[get_args_parser()])
