...
class FullyConnectedTensorProductRescaleNormSwishGate(FullyConnectedTensorProductRescaleNorm):
    ...
    def __init__(self, irreps_in1, irreps_in2, irreps_out,
        ...
        if irreps_gated.num_irreps == 0:
            ...
            # gate = Activation(irreps_out, acts=[torch.nn.ReLU()])
        else:
            gate = Gate(
                irreps_scalars, [torch.nn.SiLU() for _, ir in irreps_scalars],  # scalar
                # irreps_scalars, [torch.nn.ReLU() for _, ir in irreps_scalars],  # scalar
    ...
    def __init__(self, irreps_node_input, irreps_edge_attr, irreps_node_output, 
        ...
        if use_activation:
            if irreps_gated.num_irreps == 0:
                ...
                # gate = Activation(self.irreps_node_output, acts=[torch.nn.ReLU()])
            else:
                gate = Gate(
                    irreps_scalars, [torch.nn.SiLU() for _, ir in irreps_scalars],  # scalar
                    # irreps_scalars, [torch.nn.ReLU() for _, ir in irreps_scalars],  # scalar
...
class GraphAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # message drop
        self.message_drop = None
        if message_drop != 0:
            self.message_drop = torch.nn.Dropout(message_drop)
        ...
        if self.nonlinear_message:
            ...
            # self.sep_act = SeparableFCTP(self.irreps_pre_attn, self.irreps_edge_attr, self.irreps_pre_attn, fc_neurons, use_activation=True, norm_layer=None, internal_weights=False)
            self.sep_act = SeparableFCTP((self.irreps_pre_attn + self.irreps_pre_attn).simplify(), self.irreps_edge_attr, self.irreps_pre_attn, fc_neurons, use_activation=True, norm_layer=None, internal_weights=False)
            # self.message_norm = EquivariantLayerNormV2(self.sep_act.dtp.irreps_out)
            self.sep_alpha = LinearRS(self.sep_act.dtp.irreps_out, irreps_alpha)
            self.sep_value = SeparableFCTP(self.irreps_pre_attn, self.irreps_edge_attr, irreps_attn_heads, fc_neurons=None, use_activation=False, norm_layer=None, internal_weights=True)
            # self.sep_value = SeparableFCTP((self.irreps_pre_attn + self.irreps_pre_attn).simplify(), self.irreps_edge_attr, irreps_attn_heads, fc_neurons=None, use_activation=False, norm_layer=None, internal_weights=True)
            ...
            # self.sep = SeparableFCTP(self.irreps_pre_attn, self.irreps_edge_attr, irreps_attn_all, fc_neurons, use_activation=False, norm_layer=None)
            self.sep = SeparableFCTP((self.irreps_pre_attn + self.irreps_pre_attn).simplify(), self.irreps_edge_attr, irreps_attn_all, fc_neurons, use_activation=False, norm_layer=None)
        ...
        self.value_dot = torch.nn.Parameter(torch.randn(1, num_heads, self.irreps_head.dim))
        torch_geometric.nn.inits.glorot(self.value_dot) # Following GATv2

        self.gamma_1 = torch.nn.Parameter(1 * torch.ones((self.irreps_head.dim)),requires_grad=True)
    ...
    def get_index(self, seq_len):
        import numpy as np
        index = np.pi / 2 * torch.arange(1, seq_len + 1).reshape( -1, 1)
        return torch.nn.Parameter(index, requires_grad=False)
        
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_scalars, 
        batch, eps=1e-6,**kwargs):
        ...
        message = torch.cat((message_src[edge_src],message_dst[edge_dst]), dim=-1)

        # message drop
        # if self.message_drop != None:
        #     message = self.message_drop(message)
            
        # print("message.shape",message.shape)
        # print(message[0])
        # weight_index = self.get_index(message.size()[0]).to(message)
        # print("weight_index.shape",weight_index.shape)
        # message = message*torch.sin(weight_index[:message.size()[0],:])+message*torch.cos(weight_index[:message.size()[0],:])
        # print("message.shape",message.shape)
        # print(message[0])
        ...
        value_mul = torch.einsum('bik, aik -> bi', value, self.value_dot)
        value_mul = torch_geometric.utils.softmax(value_mul, edge_dst)
        alpha = alpha + value_mul
        # alpha = alpha/torch.clamp_min(alpha.sum(dim=-1, keepdim=True), eps)
        
        # alpha = self.gamma_1 * alpha.unsqueeze(-1)
        ...
        # node_output = node_output/torch.clamp_min(node_output.sum(dim=-1, keepdim=True), eps)
        return node_output
...
class TransBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # self.norm_2 = torch.nn.BatchNorm1d(self.irreps_node_input.dim)

        self.gamma_1 = torch.nn.Parameter(1 * torch.ones((self.irreps_node_input.dim)),requires_grad=True)
        self.gamma_2 = torch.nn.Parameter(1 * torch.ones((self.irreps_node_output.dim)),requires_grad=True)
        ...
        self.ffn = FeedForwardNetwork(
            ...
            proj_drop=proj_drop)
        self.norm_3 = torch.nn.BatchNorm1d(self.irreps_node_output.dim)
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_scalars, 
        ...
        # node_features = self.norm_1(node_features)
        ...
        node_features = self.ga(node_input=node_features, 
            ...
            batch=batch)
        node_features = self.gamma_1 * node_features
        ...
        # node_features = self.norm_2(node_features)
        # print("node features",node_features.shape)
        # print("gamma2",self.gamma_2.shape)
        node_features = self.ffn(node_features, node_attr)
        # print("node features",node_features.shape)
        # print("gamma2",self.gamma_2.shape)
        # import sys
        # sys.exit()
        node_features = self.gamma_2 * node_features
        node_features = self.act(self.norm_3(node_features))
...
class NodeEmbeddingNetwork(torch.nn.Module):
    ...
    def __init__(self, irreps_node_embedding, max_atom_type=_MAX_ATOM_TYPE, norm_layer="layer",bias=True):
        
        super().__init__()
        self.max_atom_type = 92
        # self.max_atom_type = 438#cfid, but mae bad
        self.irreps_node_embedding = o3.Irreps(irreps_node_embedding)
        self.atom_type_lin = LinearRS(o3.Irreps('{}x0e'.format(self.max_atom_type)), 
            self.irreps_node_embedding, bias=bias)
        self.atom_type_lin.tp.weight.data.mul_(self.max_atom_type ** 0.5)
        # self.norm = get_norm_layer(norm_layer)(self.irreps_node_embedding)#add for cfid
        
    def forward(self, node_atom, f_in):
        '''
            `node_atom` is a LongTensor.
        '''
        
        node_attribute = torch.tensor(f_in,dtype = torch.float32)
        # print(node_attribute[0])
        node_embedding = self.atom_type_lin(node_attribute)
        # node_embedding = self.norm(node_embedding)
        # print("node_embedding[0]",node_embedding[0])
        # import sys
        # sys.exit()
        node_attr = torch.nn.functional.one_hot(node_atom, _MAX_ATOM_TYPE).float()
...
class ScaledScatter(torch.nn.Module):
    ...
    def forward(self, x, index, **kwargs):
        out = scatter(x, index, reduce='add',**kwargs)
        #print(index)
        #print(out)
...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        # self.norm = torch.nn.BatchNorm1d(self.irreps_feature.dim)
        ...
        self.head = torch.nn.Sequential(
            ...
            # Activation(self.irreps_feature, acts=[torch.nn.ReLU()]),
    ...
    def build_blocks(self):
        for i in range(self.num_layers):
            if i != (self.num_layers - 1):
                ...
                irreps_block_output =(self.irreps_node_embedding+self.irreps_node_embedding).simplify()
            else:
                irreps_block_output = self.irreps_feature
                
            blk = TransBlock(
                # irreps_node_input=self.irreps_node_embedding, 
                irreps_node_input=(self.irreps_node_embedding+self.irreps_node_embedding).simplify(),
                irreps_node_attr=self.irreps_node_attr,
                irreps_edge_attr=self.irreps_edge_attr, 
    ...
    def forward(self, data):
        ...
        if self.use_atom_edge_attr:
            ...
            edge_length_embedding = torch.cat((src_attr[edge_src], dst_attr[edge_dst], edge_length_embedding), dim=1)
...
def graph_attention_transformer_nonlinear_l2_e3(irreps_in, radius, num_basis=128, 
    ...
    model = GraphAttentionTransformer(
        ...
        # alpha_drop=0.1, proj_drop=0.2, out_drop=0.4, drop_path_rate=0.4,
        # drop path rate : in transblock after ffn
        # out_drop: in GraphAttentionTransformer, after norm(blk)
        # proj_drop: in ga, ffn, before output
