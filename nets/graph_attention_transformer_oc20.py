...
from .graph_attention_transformer import (get_norm_layer, 
    ...
    TransBlock, 
    EmbeddingNetwork, EdgeDegreeEmbeddingNetwork, ScaledScatter
)
...
class GraphAttentionTransformerOC20(torch.nn.Module):
    ...
    def __init__(self,
        ...
        use_atom_edge_attr=False, irreps_atom_edge_attr='1x0e',
        irreps_feature='512x0e+256x1e',
        irreps_head='32x0e+16x1e', num_heads=8, irreps_pre_attn='256x0e+128x1e',
        rescale_degree=False, nonlinear_message=True,
        irreps_mlp_mid='768x0e+384x1e',
        norm_layer='layer',
        alpha_drop=0.2, proj_drop=0.0, out_drop=0.0, drop_path_rate=0.05,
        use_auxiliary_task=True, auxiliary_head_dropout=True,
        use_attention_head=False, 
        otf_graph=True, use_pbc=True, max_neighbors=500,task_mean=0, 
        task_std=0):
        ...
        self.atom_embed = EmbeddingNetwork(self.irreps_node_embedding, _MAX_ATOM_TYPE)
        self.tag_embed = EmbeddingNetwork(self.irreps_node_embedding, _NUM_TAGS)
        

        self.attr_embed = None
        if self.use_node_attr:
            self.attr_embed = EmbeddingNetwork(self.irreps_node_attr, _MAX_ATOM_TYPE)
        ...
        if self.use_atom_edge_attr:
            self.edge_src_embed = EmbeddingNetwork(self.irreps_atom_edge_attr, _MAX_ATOM_TYPE)
            self.edge_dst_embed = EmbeddingNetwork(self.irreps_atom_edge_attr, _MAX_ATOM_TYPE)
...
@register_model
def graph_attention_transformer_oc20(radius, num_basis=128, 
    task_mean=None, task_std=None, **kwargs):
    model = GraphAttentionTransformerOC20(
        irreps_node_embedding='256x0e+128x1e', num_layers=5,
        irreps_node_attr='1x0e', irreps_sh='1x0e+1x1e',
        use_node_attr=True,max_radius=radius,
        number_of_basis=num_basis, fc_neurons=[64, 64], 
        use_atom_edge_attr= False,irreps_atom_edge_attr='1x0e',
        irreps_feature='512x0e+256x1e',
        irreps_head='32x0e+16x1e', num_heads=8, 
        irreps_pre_attn='256x0e+128x1e',
        rescale_degree=False, nonlinear_message=True,
        irreps_mlp_mid='768x0e+384x1e',
        norm_layer='layer',
        alpha_drop=0.1, proj_drop=0.0, out_drop=0.0, drop_path_rate=0.1,otf_graph=True,use_pbc=True,max_neighbors=9,use_auxiliary_task=True,task_mean=task_mean, 
        task_std=task_std)
    return model
